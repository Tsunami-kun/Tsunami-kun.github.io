...
Network は普通、グラフやハイパーグラフとして導入され、undergrad にとっても比較的わかりやすい。たとえば上海科技大学では、CS の学生は一年生の後期に離散数学を学び、$G=\{N,E\}$ や最大流最小カットのような対象を早い段階で考え始める。しかし、複雑ネットワークはそんな trivial な定義で尽きるものではない。私にとって重要だった観察はこうだ。複雑ネットワークは一つの trinity であり、同時に数学的構造であり、計算過程であり、論理体系でもある。当時の私は PL theory の PhD を考えていて、この insight の背後には Curry–Howard–Lambek correspondence があるのではないか、あるいは PL theory が本体論のための形式的な骨格を与えうるのではないか、と考えていた。
...
它的路径范畴 $\mathbf{Path}(G)$ 是自由范畴——节点为对象，路径为态射，路径拼接为复合。对DAG，从 $u$ 到 $v$ 的路径总数由矩阵 $(I-A)^{-1}$ 的 $(u,v)$ 项给出。给每个节点赋一个类型 $A_v$（状态空间），每条边赋一个类型化函数 $f_{uv}: A_u \to A_v$，整个网络就是一个类型化函数的图（diagram）——或等价地，一个λ-演算 $\Lambda_G$ 中的项。关键约束是：应用（application）只能沿已有的边进行。$\beta$-归约就是消息传递。在 Curry–Howard 之下，每个类型 $A_u \to A_v$ 是一个命题："存在从 $u$ 的状态到 $v$ 的状态的变换。"良类型项是它的证明。可达性等于可证性。多条路径等于多个证明。证明规范化等于路径简化。

**EN** |
A directed network $G$ naturally inhabits all three columns. Its path category $\mathbf{Path}(G)$ is a free category: nodes are objects, paths are morphisms, and path concatenation is composition. For a DAG, the total number of paths from $u$ to $v$ is given by the $(u,v)$-entry of the matrix $(I-A)^{-1}$. Assign a type $A_v$ to each node (its state space), assign a typed function $f_{uv}: A_u \to A_v$ to each edge, and the whole network becomes a diagram of typed functions — equivalently, a term in a λ-calculus $\Lambda_G$. The key restriction is that application can only happen along existing edges. $\beta$-reduction is message passing. Under Curry–Howard, each type $A_u \to A_v$ is a proposition: “there exists a transformation from the state of $u$ to the state of $v$.” A well-typed term is its proof. Reachability becomes provability. Multiple paths become multiple proofs. Proof normalization becomes path simplification.

**日本語** |
有向ネットワーク $G$ は自然にこの三列のすべてに住んでいる。経路圏 $\mathbf{Path}(G)$ は自由圏であり、ノードが対象、経路が射、経路の連結が合成になる。DAG の場合、$u$ から $v$ への経路総数は行列 $(I-A)^{-1}$ の $(u,v)$ 成分で与えられる。各ノードに型 $A_v$（状態空間）を与え、各辺に型付き関数 $f_{uv}: A_u \to A_v$ を与えると、ネットワーク全体は型付き関数の図式、あるいは λ 計算 $\Lambda_G$ の項として読める。重要な制約は、application が既存の辺に沿ってしか起きないことだ。$\beta$ 簡約はメッセージ伝達になる。Curry–Howard の下では、各型 $A_u \to A_v$ は「$u$ の状態から $v$ の状態への変換が存在する」という命題である。良い型をもつ項はその証明である。可達性は可証性になり、多重経路は複数の証明になり、証明の正規化は経路の簡約になる。
...
**中文** |
无标度网络中，枢纽节点有极高的度，而大多数节点度为1或2。这种异质性要求依赖类型：节点的类型签名依赖于它的度——输入是依赖积（所有前驱必须贡献），输出是依赖和（为每条消息选择一个后继）。Barabási–Albert 偏好连接过程在类型论阅读下变成：新节点在进行*类型推断*，它通过选择与哪些已有类型复合来确定自己的类型签名。偏好连接说的是：优先与类型签名更丰富的节点复合。无标度网络的聚类系数衰减 $C(k) \sim k^{-1}$ 意味着：经过枢纽节点的证明更不容易有冗余（更少的三角形替代路径）。枢纽是*不可替代的*证明步骤——删除它们一次摧毁许多命题。这是无标度网络对定向攻击脆弱性的类型论解释。

**EN** |
In scale-free networks, hub nodes have extremely high degree while most nodes have degree 1 or 2. This heterogeneity calls for dependent types: a node's type signature depends on its degree. Inputs become dependent products (all predecessors must contribute), and outputs become dependent sums (a successor is chosen for each message). Under this reading, the Barabási–Albert preferential attachment process becomes a form of *type inference*: a new node determines its own type signature by choosing which existing types to compose with. Preferential attachment says: compose first with nodes that have richer type signatures. The decay of the clustering coefficient, $C(k) \sim k^{-1}$, means that proofs passing through hubs are less likely to be redundant. Hubs become *irreplaceable* proof steps, and removing them destroys many propositions at once. This is the type-theoretic explanation of the fragility of scale-free networks under targeted attack.

**日本語** |
無標度ネットワークでは、ハブノードが非常に高い次数を持つ一方で、大多数のノードの次数は 1 か 2 である。この異質性は依存型を要求する。すなわち、ノードの型シグネチャがその次数に依存するのである。入力は依存積（すべての前駆が寄与しなければならない）になり、出力は依存和（各メッセージに対して一つの後継が選ばれる）になる。この読みの下では、Barabási–Albert の優先的選択過程は*型推論*に見える。新しいノードは、どの既存の型と合成するかを選ぶことで自分の型シグネチャを決めている。優先的選択が言っているのは、より豊かな型シグネチャをもつノードと優先的に合成せよ、ということである。さらに、クラスタリング係数の減衰 $C(k) \sim k^{-1}$ は、ハブを通る証明が冗長になりにくいことを意味する。ハブは*代替不可能な*証明ステップになり、それらを削除すると多くの命題が一度に壊れる。これは、無標度ネットワークが標的型攻撃に脆いことの型理論的説明である。
...
**中文** |
Watts–Strogatz 小世界模型中，中等重连概率 $p_{\text{rw}}$ 下，网络同时具有高聚类和短平均路径。在证明空间中：局部证明高度结构化（多条经由三角形的替代证明——证明冗余性），同时远程证明通过随机捷径变得可用。小世界性质是证明鲁棒性与证明效率的共存——这或许也是设计良好的逻辑系统（和运转良好的社会）的特征。Granovetter 所说的"弱连接的力量"，在 Curry–Howard 下就是：一个弱连接是原本需要很长证明的命题的一个短证明。

**EN** |
In the Watts–Strogatz small-world model, an intermediate rewiring probability $p_{\text{rw}}$ gives a network both high clustering and short average path length. In proof space this means: local proofs remain highly structured, with many alternatives through triangles, while long-range proofs become available through random shortcuts. The small-world property is the coexistence of proof robustness and proof efficiency — perhaps also the characteristic feature of a well-designed logical system, or even of a well-functioning society. What Granovetter called “the strength of weak ties” becomes, under Curry–Howard, the fact that a weak tie is a short proof of a proposition that would otherwise require a very long proof.

**日本語** |
Watts–Strogatz の小世界モデルでは、中程度の再配線確率 $p_{\text{rw}}$ のもとで、ネットワークは高いクラスタリングと短い平均経路長を同時にもつ。証明空間で言えば、局所的な証明は三角形を通る多数の代替経路によって高く構造化されたままであり、その一方で遠距離の証明はランダムなショートカットによって利用可能になる。小世界性とは、証明のロバスト性と証明の効率の共存である。これは、よく設計された論理体系や、よく機能する社会の特徴でもあるかもしれない。Granovetter のいう「弱い紐帯の強さ」は、Curry–Howard の下では、本来なら非常に長い証明を要する命題に対する短い証明として読み直される。
...
**中文** |
图的度分布熵度量节点类型的多样性——在Curry–Howard下就是类型空间的熵。更精细的不变量是 von Neumann 熵，由归一化拉普拉斯矩阵的特征值定义，度量证明空间支持多少独立的推理模式。节点间互信息的证明论解释：它度量涉及 $u$ 的证明在多大程度上约束涉及 $v$ 的证明。这连接到相关逻辑——有效证明必须*使用*其假设。当互信息为零，两处的证明独立；当互信息高，它们是证明论意义上纠缠的。

**EN** |
The entropy of a graph's degree distribution measures the diversity of node types; under Curry–Howard it becomes the entropy of the type space itself. A finer invariant is the network's von Neumann entropy, defined from the eigenvalues of the normalized Laplacian, which measures how many independent modes of inference the proof space can support. Mutual information between two nodes has a proof-theoretic reading as well: it tells us how strongly a proof involving $u$ constrains possible proofs involving $v$. This links network theory to relevance logic, where a valid proof must *use* its hypotheses. When mutual information is zero, the two proof spaces are independent; when it is high, they are entangled in a proof-theoretic sense.

**日本語** |
グラフの次数分布エントロピーはノード型の多様性を測るが、Curry–Howard の下ではそれは型空間のエントロピーになる。さらに精密な不変量として、正規化ラプラシアンの固有値から定義される von Neumann エントロピーがあり、証明空間がどれだけ多くの独立した推論モードを支えられるかを測る。二つのノードのあいだの相互情報量にも証明論的な読みがある。それは、$u$ を含む証明が $v$ を含む証明の可能性をどれほど制約するか、という量である。これは関連論理とつながる。関連論理では、有効な証明はその仮定を*実際に使わなければならない*。相互情報量がゼロなら二つの証明空間は独立であり、高ければ証明論的な意味で絡み合っている。
