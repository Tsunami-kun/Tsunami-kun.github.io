...
excerpt: "A note on reading complex networks as mathematical structures, computational processes, and logical systems."
...
**中文** |
我重构这篇 2021 年的文稿，是因为最近又回到一个相近的问题：在多智能体和人机协作系统里，网络中的部分与整体到底该怎么理解？社区是网络的*部分*，还是网络由社区*组成*？在范畴论里，子对象和商是对偶构造，所以形式上两种说法都成立；但这并没有真正回答问题。这篇文稿里的想法还不完整，更像一个 proof sketch。对我来说，它真正留下来的问题是：如果一个网络可以同时被读作数学结构、计算过程和逻辑系统，那么这到底是在说明某种本体事实，还是只是在说明一种强有力的形式框架？我现在还没有结论，但这个问题本身仍然值得保留。

**EN** |
I rebuilt this 2021 manuscript because a related question returned in a newer setting: in multi-agent systems and human-AI collaboration, how should we understand part-whole relations in networks? Is a community a *part* of a network, or is the network a *composition* of communities? In category theory the formal answer is both, because subobjects and quotients are dual constructions, but that does not really settle the issue. The ideas in this manuscript are still incomplete; at best it is a proof sketch. The question I keep is simpler: if a network can be read at once as a mathematical structure, a computational process, and a logical system, does that point to an ontological fact, or only to a strong formal framework? I do not have a final answer yet, but the question is still worth keeping.

**日本語** |
私がこの 2021 年の原稿を組み直したのは、最近また近い問いに戻ってきたからである。多エージェント系や人間と AI の協働では、ネットワークにおける部分と全体の関係を改めて考える必要がある。コミュニティはネットワークの*部分*なのか、それともネットワークはコミュニティの*組成*なのか。圏論では、子対象と商が双対構成なので、形式上の答えは両方になる。だが、それで問題が解決するわけではない。この原稿のアイデアはまだ未完成で、せいぜい proof sketch にとどまる。私が残したい問いはもっと単純である。ネットワークを数学的構造、計算過程、論理体系として同時に読めるとき、それは本体論的な事実を示しているのか、それとも強い形式的枠組みを示しているだけなのか。まだ結論はないが、この問いは残しておきたい。
